...
- (void)requestPublicIPandPortWithUDPSocket:(GCDAsyncUdpSocket *)socket delegate:(id<STUNClientDelegate>)_delegate{
    ...
    //
    [socket sendData:stunRequest toHost:STUNServer port:STUNPort withTimeout:10 tag:1002];
}
...
- (void)sendIndicationMessage{
    ...
    //STUNLog(@"STUN Indication Message=%@ %d", stunIndicationMessage, [udpSocket localPort]);
